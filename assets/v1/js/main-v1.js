(() => {
  ...
  const setupStatusNotifications = function () {
    ...
    window.showStatus = (message, type = 'online', duration = 8000) => {
      ...
      statusEl.innerHTML ='<div class="status-icon flex-shrink-0" style="width: 32px; height: 32px;"></div>' + '<div class="ms-3 fw-medium">' + message + '</div>';
      ...
      setTimeout(() => {
        ...
        statusEl.addEventListener('transitionend', () => statusEl.remove(), {
          once: true,
        });
      }, duration);
    };
    ...
    const handleOffline = function () {
      ...
      if (!stillOfflineInterval) {
        stillOfflineInterval = setInterval(() => {
          if (!navigator.onLine) {window.showStatus("You're still offline.", 'still-offline', 5000);
          } else {
            handleOnline();
          }
        }, 20000);
