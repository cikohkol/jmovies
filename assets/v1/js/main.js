(() => {
  ...
  const setupStatusNotifications = () => {
    const statusContainer = document.getElementById('status-container');
    window.showStatus = (message, type = 'online', duration = 4000) => {
      while (statusContainer.firstChild) {
        statusContainer.removeChild(statusContainer.firstChild);
      }
      const statusEl = document.createElement('div');
      statusEl.className = 'status d-flex align-items-center p-3 rounded-3 shadow-lg ' + type;
      statusEl.innerHTML =
        '<div class="status-icon flex-shrink-0" style="width: 32px; height: 32px;"></div>' +
    ...
    const handleOnline = () => window.showStatus('Connected. You are back online!', 'online');
    const handleOffline = () => window.showStatus('Connection lost. You are now offline.', 'offline', 5000);
    window.addEventListener('online', handleOnline);
    window.addEventListener('offline', handleOffline);
    if (!navigator.onLine) {
      window.showStatus("You're currently offline.", 'still-offline', 5000);
    }
  };
  ...
  const initializePWA = function () {
    ...
    const hideBanner = (isPermanent = false) => {
      ...
      installBanner.addEventListener(
        'transitionend',
  ...
  const setupLanguageSwitcher = function () {
    ...
    if (currentLangElement) {
      ...
      if (flag && name) {langDropdownButton.innerHTML = '<span class="fi fi-' + flag +'"></span><span class="ms-2">' + name +'</span>';
      }
    }
  ...
  const setupInfiniteScroll = function () {
    ...
    const loadMoreContent = async () => {
      ...
      isLoading = true;
      loaderContainer.innerHTML = '<div class="spinner-border text-secondary" role="status"><span class="sr-only"></span></div>';
  ...
  const setupTvDetailTabs = function () {
    ...
    seasonsTabPane.addEventListener('click', async (e) => {
      ...
      if (seasonItem) {
        ...
        episodesTabPane.innerHTML = '<div class="d-flex justify-content-center mt-4"><div class="spinner-border text-secondary" role="status"><span class="visually-hidden">Loading...</span></div></div>';
        ...
        try {
          const partialUrl =
            window.location.origin + '/tv/' + tvId + '/season/' + seasonNumber + '?lang=' + lang + '&partial=true';
      ...
      if (loadMoreSeasonsBtn) {
        e.preventDefault();
        seasonsTabPane
          .querySelectorAll('.season-item.d-none')
          .forEach((item) => {
    ...
    episodesTabPane.addEventListener('click', (e) => {
      ...
      if (episodeLink) {
        ...
        episodesTabPane
          ...
          .forEach((link) => link.closest('li').classList.remove('active'));
    ...
    document.body.addEventListener('click', (e) => {
      ...
      if (loadMoreEpisodesBtn) {
        ...
        if (list) {
          list
            .querySelectorAll('[data-episode-item].d-none')
            .forEach((item) => {
              item.classList.remove('d-none');
            });
        }
...
window.play = (videoSrc) => {
  ...
  const handleModalClose = () => {
    // Show modal again after the same 10 second duration
    setTimeout(showModal, 10000);
  };

  // Initial modal show after 10 seconds
  setTimeout(showModal, 10000);
};
